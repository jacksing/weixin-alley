...
"""
    ...
    website more conveniently, through weixin login entrance.
...
WEIXIN_AUTH_DOMAIN = 'open.weixin.qq.com'
WEIXIN_QRIMG_REGEX = r'img .* src="(.*)"'
WEIXIN_SITENAME_REGEX = r'id="wx_default_tip"[\s\S]*?<p>[\s\S]*?<p>(.*)</p>'
...
USER_KEY_NAME = 'WEIXIN-LOGIN-USER'
...
def index():
    ...
    if request.method == 'GET':
        if user_config.is_authenticated:
            ...
            tokens are stored by weixin-login backend. So here providing a link
            ...
            '''
            Visit weixin QR scan page to get the QR image and site name.
            After saving the necessary information, send the QR image to
            user for QR scaning.

            Keep the Regular Expressions in maintenance in case of weixin
            changes its QR scan page.
            '''
            qr_page = requests.get(_get_weixin_qr_scan_url(user_config)).text
            qr_img = urljoin(
                user_config.weixin_login_domain,
                re.findall(WEIXIN_QRIMG_REGEX, qr_page)[0],
            )
            qr_id = qr_img.split('/')[-1]
            site_name = re.findall(WEIXIN_SITENAME_REGEX, qr_page)[0]
        ...
        try:
            ...
            '''If the given url is not a valid weixin QR scan page url,
            visit the given url and try to get the real QR scan url.'''
            if parsed.netloc != WEIXIN_AUTH_DOMAIN:
                ...
                if parsed.netloc != WEIXIN_AUTH_DOMAIN:
                    user_config.clear()
                    flash('The url given is not supported to do weixin login.')
                    flash(qr_url)
                    parsed = None
                else:
            ...
            if parsed:
                ...
                user_config.set(
                    ...
                    state=list(query_dict['state'])[0].replace('#weixin_redirect', ''),
...
def _get_weixin_qr_scan_url(user_config):
    return (
        'https://'
        '{weixin_auth_domain}/connect/qrconnect'
        '?appid={appid}'
        ...
        weixin_auth_domain=WEIXIN_AUTH_DOMAIN,
...
def query(qr_id, last=None):
    ...
    weixin_resp = requests.get(query_url)
    resp_text = weixin_resp.text
